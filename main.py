...
extend_model_data = copy.deepcopy(model_data)
indexies = np.where(data[:, 0] < x_diff * (n - 1))
for index in indexies[0]:
    extend_model_data.append(model_data[index])
...
for i in range(n):
    ...
    ideal_coordinates[(ideal_coordinates[:, 1] > max_y), 1] = min_y + (ideal_coordinates[(ideal_coordinates[:, 1] > max_y), 1] - max_y)
...
for row in pairing_index:
    pairing_model = [extend_model_data[i] for i in row]
    
    print([m.id for m in pairing_model])
